...
<div class="max-w-7xl mx-auto pb-12 mt-6 fade-in">
    {{-- Header --}}
    <div class="mb-6">
        <a href="/" class="text-slate-500 hover:text-brand-900 flex items-center gap-2 mb-4 transition-colors text-sm">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 mb-8">
        ...
        <div class="mt-4 pt-4 border-t border-slate-200">
            <p class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-2">Catatan Khusus</p>
            <p class="text-sm text-slate-700 font-medium">⚠️ {{ $spk->notes }}</p>
        </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 mb-8">
        <h2 class="text-base font-bold text-slate-900 mb-5">Alur Proses Produksi</h2>
        <div class="flex items-center gap-0 overflow-x-auto">
            @php
                $allSteps = ['cleaning' => '1. Cleaning (Awal)', 'transfer' => '2. Transfer', 'weighing' => '3. Penimbangan', 'mixing' => '4. Mixing', 'extruding' => '5. Extruding'];
                $currentIdx = array_search($spk->current_step, array_keys($allSteps));
            @endphp
            @foreach($allSteps as $key => $label)
            @php 
                $idx = array_search($key, array_keys($allSteps)); 
                $isDone = $idx < $currentIdx; 
                $isCurrent = $key === $spk->current_step; 
                $isPartial = false;

                // Determine if clickable based on prerequisites
                $canClick = false;
                if ($key === 'cleaning') {
                    $canClick = true;
                } elseif ($key === 'transfer') {
                    $canClick = $spk->cleaningLogs->count() > 0 || $currentIdx >= 1;
                } elseif ($key === 'weighing') {
                    $canClick = $spk->transferLogs->count() > 0;
                } elseif ($key === 'mixing') {
                    $canClick = $spk->batches->where('step', 'weighing')->count() > 0;
                } elseif ($key === 'extruding') {
                    $canClick = $spk->batches->where('step', 'mixing')->count() > 0;
                }

                // Check for partial completion
                if ($isDone || $isCurrent || $canClick) {
                    if ($key === 'transfer') {
                        foreach ($spk->materials as $mat) {
                            $transferred = $spk->transferLogs->where('material_name', $mat->material_name)->sum('weight_kg');
                            if ($transferred < $mat->total_kg) { $isPartial = true; break; }
                        }
                    } elseif ($key === 'weighing' || $key === 'mixing') {
                        $reqBatches = $spk->materials->max('total_batches_required') ?? 0;
                        $doneBatches = $spk->batches->where('step', $key)->count();
                        if ($doneBatches < $reqBatches) { $isPartial = true; }
                    }
                }
                ...
                // Check if step has active emergency report
                $hasEmergency = \App\Models\EmergencyReport::where('spk_id', $spk->id)
                    ->where('step', $key)
                    ->where('status', '!=', 'resolved')
                    ->exists();
                
                if ($hasEmergency) {
                    $circleClass = 'bg-red-50 text-red-500 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)]';
                    $lineClass = 'bg-red-500/50 dashed border-t-2 border-red-500/50 bg-transparent h-0';
                    $textClass = 'text-red-500';
                } elseif ($isDone && !$isPartial) {
                    $circleClass = 'bg-green-500 border-green-500 text-white shadow-[0_0_10px_rgba(34,197,94,0.3)]';
                    $lineClass = 'bg-green-500';
                    $textClass = 'text-green-400';
                } elseif (($isDone || $canClick) && $isPartial) {
                    $circleClass = 'bg-amber-500/20 border-amber-500 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.2)]';
                    $lineClass = 'bg-amber-500/50 dashed border-t-2 border-amber-500/50 bg-transparent h-0';
                    $textClass = 'text-amber-400';
                } elseif ($isCurrent || $canClick) {
                    $circleClass = 'bg-brand-500 border-brand-500 text-white shadow-[0_0_15px_rgba(14,165,233,0.4)]';
                    $textClass = 'text-brand-400';
                }
            @endphp
            <div class="flex items-center flex-shrink-0">
                <a href="{{ $canClick ? '/production/'.$key.'?spk_id='.$spk->id : '#' }}" class="flex flex-col items-center {{ $canClick ? 'cursor-pointer hover:opacity-80 transition-opacity' : 'cursor-not-allowed opacity-50' }}">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold border-2 {{ $circleClass }}">
                        @if($hasEmergency)
                    ...
                    <p class="text-xs mt-2 font-medium {{ $textClass }} whitespace-nowrap">
                        {{ explode('. ', $label)[1] }}
                        @if($isPartial)<span class="block text-[10px] opacity-80 text-center mt-0.5">(Partial)</span>@endif
                    </p>
                </a>
                @if(!$loop->last)
                <div class="w-12 mx-1"{{ $lineClass }} flex-shrink-0 -mt-6 {{ str_contains($lineClass, 'dashed') ? 'border-dashed' : 'h-0.5' }}></div>
                @endif
            </div>
            @endforeach
        </div>
    ...
    <div class="flex justify-end">
        <a href="/production/{{ $spk->current_step ?? 'transfer' }}?spk_id={{ $spk->id }}">
            <button class="flex items-center gap-3 px-10 py-5 rounded-2xl bg-brand-500 hover:bg-brand-400 text-white font-extrabold text-xl shadow-[0_0_30px_rgba(14,165,233,0.4)] transition-all">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                Mulai Proses → {{ strtoupper(str_replace('_', ' ', $spk->current_step ?? 'Transfer')) }}
            </button>
        </a>
    </div>
</div>
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('spkReport', () => {
        ...
        return {
            ...
            if (step === 'penimbangan' && this.chartData.weighing && this.chartData.weighing.completed > 0) {
            ...
            if (step === 'mixing' && this.chartData.mixing && this.chartData.mixing.completed > 0) {
            ...
            if (step === 'extruder' && this.chartData.extruding && this.chartData.extruding.completed > 0) {
