...
<div class="fade-in">
    <div class="mb-6">
        <h1 class="text-3xl font-extrabold text-slate-900 mb-1">Pencarian SPK</h1>
    ...
    <div class="flex items-center gap-2 mb-6 flex-wrap">
        @php
            $tabs = [
                'all'      => ['label' => 'Semua',          'count' => $spks->count()],
                'upcoming' => ['label' => 'Mendatang',       'count' => $spks->where('status','upcoming')->count()],
                'ongoing'  => ['label' => 'Sedang Berjalan', 'count' => $spks->whereIn('status',['ongoing','waiting_approval'])->count()],
                'done'     => ['label' => 'Selesai',         'count' => $spks->where('status','done')->count()],
            ];
        @endphp
    ...
    <div id="spk-list" class="space-y-4">
        ...
        @php
            ...
            // Extruding progress â€” based on extrudingLogs
            $extrudingDone = $spk->extrudingLogs->count() > 0 ? 100 : 0;

            // Cleaning progress â€” based on cleaningLogs
